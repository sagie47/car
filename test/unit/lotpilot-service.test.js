...
test('inventory ingest upserts by VIN and creates a listing draft', async () => {
  const { service } = createServiceContext();
  const { rooftop } = await createDealerContext(service);

  const firstIngest = await service.ingestInventory({
  ...
  const [updatedVehicle] = await service.listVehicles({ rooftopId: rooftop.id });
  assert.equal((await service.listVehicles({ rooftopId: rooftop.id })).length, 1);
  assert.equal(updatedVehicle.price, 24995);
});

test('duplicate VINs are flagged and not imported twice in the same batch', async () => {
  const { service } = createServiceContext();
  const { rooftop } = await createDealerContext(service);

  const result = await service.ingestInventory({
    rooftopId: rooftop.id,
    sourceType: 'csv',
...
test('sold inventory queues an existing published listing for removal', async () => {
  const { service } = createServiceContext();
  const { rooftop } = await createDealerContext(service);
...
test('lead assignment and status updates are tracked', async () => {
  ...
  const staleVehicles = await service.listStaleVehicles(rooftop.id);
  ...
  const assignedLead = await service.assignLead(lead.id, 'rep_123', 'manager');
  const respondedLead = await service.updateLeadStatus(lead.id, 'responded', { actor: 'rep_123' });
