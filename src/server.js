...
import { LotPilotService } from './service/lotpilot-service.js';
...
async function handleRequest(request, response) {
  ...
  try {
    ...
    if (request.method === 'POST' && pathname === '/api/dealers') {
      return sendJson(response, 201, service.createDealer(await readJson(request)));
    }

    if (request.method === 'POST' && pathname === '/api/rooftops') {
      return sendJson(response, 201, service.createRooftop(await readJson(request)));
    }

    if (request.method === 'POST' && pathname === '/api/ingest') {
      return sendJson(response, 200, service.ingestInventory(await readJson(request)));
    }

    if (request.method === 'GET' && pathname === '/api/vehicles') {
      return sendJson(response, 200, service.listVehicles({ rooftopId: searchParams.get('rooftopId') }));
    }

    const vehicleMatch = matchPath(pathname, '/api/vehicles/:vehicleId');
    if (request.method === 'GET' && vehicleMatch) {
      return sendJson(response, 200, service.getVehicle(vehicleMatch.vehicleId));
    }

    const vehicleDraftMatch = matchPath(pathname, '/api/vehicles/:vehicleId/draft');
    if (request.method === 'POST' && vehicleDraftMatch) {
      return sendJson(response, 200, service.generateListingDraft(vehicleDraftMatch.vehicleId, await readJson(request)));
    }

    const rooftopHealthMatch = matchPath(pathname, '/api/rooftops/:rooftopId/health');
    if (request.method === 'GET' && rooftopHealthMatch) {
      return sendJson(response, 200, service.getRooftopHealth(rooftopHealthMatch.rooftopId));
    }

    const staleVehicleMatch = matchPath(pathname, '/api/rooftops/:rooftopId/stale-vehicles');
    if (request.method === 'GET' && staleVehicleMatch) {
      return sendJson(response, 200, service.listStaleVehicles(staleVehicleMatch.rooftopId));
    }

    if (request.method === 'GET' && pathname === '/api/listings') {
      return sendJson(response, 200, service.listListings({ rooftopId: searchParams.get('rooftopId') }));
    }

    const listingMatch = matchPath(pathname, '/api/listings/:listingId');
    if (request.method === 'GET' && listingMatch) {
      return sendJson(response, 200, service.getListing(listingMatch.listingId));
    }
    ...
    if (request.method === 'POST' && listingTransitionMatch) {
      ...
      return sendJson(
        response,
        200,
        service.transitionListing(listingTransitionMatch.listingId, body.toState, {
          actor: body.actor,
          reason: body.reason,
    ...
    if (request.method === 'POST' && pathname === '/api/leads') {
      return sendJson(response, 201, service.createLead(await readJson(request)));
    }

    const leadAssignMatch = matchPath(pathname, '/api/leads/:leadId/assign');
    if (request.method === 'PATCH' && leadAssignMatch) {
      const body = await readJson(request);
      return sendJson(response, 200, service.assignLead(leadAssignMatch.leadId, body.assignedRepId, body.actor));
    }

    const leadStatusMatch = matchPath(pathname, '/api/leads/:leadId/status');
    if (request.method === 'PATCH' && leadStatusMatch) {
      const body = await readJson(request);
      return sendJson(response, 200, service.updateLeadStatus(leadStatusMatch.leadId, body.status, body));
    }
